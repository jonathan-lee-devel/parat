...
def process_build_host(build_host: dict) -> dict:
    ...
    for build_job_index in range(len(build_host[JOBS])):
        ...
        if jenkins_job_pre_run_dict is not None:
            jenkins_job_pre_run_dict_builds_sorted_by_build_number = sorted(jenkins_job_pre_run_dict[BUILDS], key=lambda x: x['number'], reverse=True)
            if jenkins_job_pre_run_dict is not None and len(jenkins_job_pre_run_dict_builds_sorted_by_build_number) > 0:
                build_number_to_track = jenkins_job_pre_run_dict_builds_sorted_by_build_number[0]['number'] + 1
